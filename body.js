...
import { Handler, Callupdate, GroupUpdate } from './scs/data/index.js';
...
const whatsappChannelId = '0029VasHgfG4tRrwjAUyTs10@newsletter'; // Ensure this is the correct format
...
async function updateLiveBio(Matrix) {
  try {
    const now = moment().tz('Africa/Nairobi');
    const time = now.format('HH:mm:ss');
    const bio = `👻Sʜᴀᴅᴏᴡ Xᴛᴇᴄʜ ꀤs Aᴄᴛɪᴠᴇ👻Aᴛ ${time}`;
    await Matrix.updateProfileStatus(bio);
  } catch (error) {
    ...
    Matrix.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (initialConnection) {
          console.log(chalk.green("👻 Sʜᴀᴅᴏᴡ Xᴛᴇᴄʜ ꀤs Nᴏᴡ Oɴʟɪɴᴇ Aɴᴅ Aʟɪᴠᴇ"));
